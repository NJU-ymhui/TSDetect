    ...
    def visit(self, node):
        ...
        if node.type == 'function_definition':
            ...
            if not self.__is_test:
                self.__non_test_table.append(self.__decl_method_name)  # 因为默认测试方法不会被调用，而是可以直接运行
        elif node.type == 'call':
            var_name = node.children[0].text
            if var_name not in self.__non_test_table:  # 之所以认为不在这里就一定未声明，是因为**默认测试方法不会被调用而是直接运行**
            ...
            if method_name in self.__methods_tobe_tested:
                ...
                if method_name == var_name:
                    ...
                    mixed = var_name + method_name
                # print("--", mixed)
                if mixed in self.__has_invoked:
                    # print(self.__has_invoked)
                    # print(self.__is_test)
                    if self.__is_test:
                        self.smell = True
                    else:
