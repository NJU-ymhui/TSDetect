def get_method_body(node):
    ...
    :return: 方法block
    """
    if node.type != 'function_definition':  # method_declaration
        return None
    i = 0
...
def get_class_body(node):
    if node.type != 'class_definition':  # class_declaration
        print("Error when get a non-class's body")
        return None
    for child in node.children:
        if child.type == 'block':  # class_body
            return child
...
def is_bool(bs):
    return bs == b'True' or bs == b'False' or bs == b'None'  # 转换后的布尔值和空值


def is_print(bs):
    return bs.startswith(b'print')  # 转换后的打印函数
...
def is_test_func(method_decl_node):
    if method_decl_node.type != 'function_definition':  # method_declaration
        return False
    for child in method_decl_node.children:
        if child.type == 'identifier':  # func name
...
def is_variable_decl(node):
    ty = node.type
    return ty == 'parameters' or ty == 'assignment'  # formal_parameter, field_declaration, local_variable_declaration
...
def is_method_decl(node):
    return node.type == 'function_definition'  # method_declaration
